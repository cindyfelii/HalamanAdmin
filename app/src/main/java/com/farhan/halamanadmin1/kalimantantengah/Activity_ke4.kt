...
class Activity_ke4 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var anggotaList : MutableList<variabel_R_kalimantantengah>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_kalimantantengah)
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (a in snapshot.children){
                        val anggota = a.getValue(variabel_R_kalimantantengah::class.java)
                        if (anggota !=null){
                            anggotaList.add(anggota)
                        }
                    }

                    val adapter = KalimantanTengahAdapter( this@Activity_ke4,
                        R.layout.activity_item_kalimantantengah, anggotaList)
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Activity_ke4, "error: "+error, Toast.LENGTH_LONG).show()
            }
        })

        listData.setOnItemClickListener { parent, view, position, id ->
            val anggota = anggotaList.get(position)
            val intent = Intent(this@Activity_ke4, TambahKalimantanTengah::class.java)
            intent.putExtra(TambahKalimantanTengah.EXTRA_ID, anggota.id)
            intent.putExtra(TambahKalimantanTengah.EXTRA_NAMA, anggota.nama)
    ...
    private fun simpanData(){
        ...
        val anggota = variabel_R_kalimantantengah(anggotaId!!, nama, alat, bahan, cara)
