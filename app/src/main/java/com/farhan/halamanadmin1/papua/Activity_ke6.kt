...
class Activity_ke6 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var anggotaList : MutableList<variabel_R_papua>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_papua)
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (a in snapshot.children){
                        val anggota = a.getValue(variabel_R_papua::class.java)
                        if (anggota !=null){
                            anggotaList.add(anggota)
                        }
                    }

                    val adapter = PapuaAdapter( this@Activity_ke6,
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Activity_ke6, "error: "+error, Toast.LENGTH_LONG).show()
            }
        })

        listData.setOnItemClickListener { parent, view, position, id ->
            val anggota = anggotaList.get(position)
            val intent = Intent(this@Activity_ke6, TambahPapua::class.java)
            intent.putExtra(TambahPapua.EXTRA_ID, anggota.id)
            intent.putExtra(TambahPapua.EXTRA_NAMA, anggota.nama)
    ...
    private fun simpanData(){
        ...
        val anggota = variabel_R_papua(anggotaId!!, nama, alat, bahan, cara)
