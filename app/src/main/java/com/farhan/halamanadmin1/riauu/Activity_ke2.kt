...
import kotlinx.android.synthetic.main.input_riau.*
import android.view.ContextMenu
import com.farhan.halamanadmin1.R

class Activity_ke2 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var anggotaList : MutableList<variabel_R_riau>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_riau)
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (a in snapshot.children){
                        val anggota = a.getValue(variabel_R_riau::class.java)
                        if (anggota !=null){
                            anggotaList.add(anggota)
                        }
                    }

                    val adapter = RiauAdapter( this@Activity_ke2,
                        R.layout.activity_item_riau, anggotaList)
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Activity_ke2, "error: "+error, Toast.LENGTH_LONG).show()
            }
        })

        listData.setOnItemClickListener { parent, view, position, id ->
            val anggota = anggotaList.get(position)
            val intent = Intent(this@Activity_ke2, TambahRiau::class.java)
            intent.putExtra(TambahRiau.EXTRA_ID, anggota.id)
            intent.putExtra(TambahRiau.EXTRA_NAMA, anggota.nama)
    ...
    private fun simpanData(){
        ...
        val anggota = variabel_R_riau(anggotaId!!, nama, alat, bahan, cara)
