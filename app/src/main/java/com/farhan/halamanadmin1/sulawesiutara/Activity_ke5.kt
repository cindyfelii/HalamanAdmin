...
class Activity_ke5 : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var anggotaList : MutableList<variabel_R_sulawesiutara>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.input_sulawesiutara)
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    for (a in snapshot.children){
                        val anggota = a.getValue(variabel_R_sulawesiutara::class.java)
                        if (anggota !=null){
                            anggotaList.add(anggota)
                        }
                    }

                    val adapter = SulawesiUtaraAdapter( this@Activity_ke5,
                        R.layout.activity_item, anggotaList)
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Activity_ke5, "error: "+error, Toast.LENGTH_LONG).show()
            }
        })

        listData.setOnItemClickListener { parent, view, position, id ->
            val anggota = anggotaList.get(position)
            val intent = Intent(this@Activity_ke5, TambahSulawesiUtara::class.java)
            intent.putExtra(TambahSulawesiUtara.EXTRA_ID, anggota.id)
            intent.putExtra(TambahSulawesiUtara.EXTRA_NAMA, anggota.nama)
    ...
    private fun simpanData(){
        ...
        val anggota = variabel_R_sulawesiutara(anggotaId!!, nama, alat, bahan, cara)
